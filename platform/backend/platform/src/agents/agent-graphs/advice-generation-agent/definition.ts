import { AIMessage } from '@langchain/core/messages';
import { MemorySaver, StateGraph } from '@langchain/langgraph';

import { createContextAwareToolActorNode } from '../../lib/context/context-aware-tool-actor.js';
import type { AgentContextManager } from '../../lib/context/manager.js';
import type { ToolCapableLlm } from '../../lib/tool-loop/tool-loop.js';
import type { PlatformDbToolContext } from '../../tool-registry/db/context.js';
import { routeAfterToolActor } from './edges.js';
import { createApologiseNode } from './nodes/apologise.js';
import { createCompileResultNode } from './nodes/compile-result.js';
import { buildAdviceGenerationToolActorPrompt } from './prompts.js';
import { AdviceGenerationState, type AdviceGenerationStateType } from './state.js';
import { createAdviceGenerationTools } from './tools.js';
import { AdviceGenerationTurnBudget } from './turn-budget.js';

/** Options for {@link buildAdviceGenerationAgent}. */
export interface AdviceGenerationAgentOptions {
    llm: ToolCapableLlm;
    checkpointer?: MemorySaver;
    dbTools: PlatformDbToolContext;
    contextManager: AgentContextManager;
    turnBudget?: AdviceGenerationTurnBudget;
}

/**
 * Builds the advice-generation task agent: read image-CVE + CVE + research docs, synthesize draft advice.
 * Internal-only — invoked via `call_advice_generation_agent` on platform-assistant or CLI.
 */
export function buildAdviceGenerationAgent({
    llm,
    checkpointer,
    dbTools,
    contextManager,
    turnBudget = new AdviceGenerationTurnBudget(),
}: AdviceGenerationAgentOptions) {
    const tools = createAdviceGenerationTools(dbTools, contextManager, turnBudget);

    const toolActor = createContextAwareToolActorNode<AdviceGenerationStateType>({
        llm,
        tools,
        buildPrompt: (state, toolList) => {
            const threadId = contextManager.getActiveThreadId() ?? 'default-thread';
            const currentLlmStep =
                state.toolMessages.filter((m) => m instanceof AIMessage).length + 1;
            return buildAdviceGenerationToolActorPrompt(
                state,
                toolList,
                turnBudget,
                threadId,
                currentLlmStep,
            );
        },
        contextManager,
        pinnedLines: (state) => [
            'agent: advice-generation-agent',
            `projectId: ${state.projectId}`,
            `componentId: ${state.componentId}`,
            `cveId: ${state.cveId}`,
            state.imageCveId.trim().length > 0 ? `imageCveId: ${state.imageCveId}` : 'imageCveId: resolve via list_image_cves',
            state.additionalContext.trim().length > 0
                ? 'additionalContext: present (see prompt section; untrusted)'
                : 'additionalContext: none',
            'output: draft advice via finish — platform-assistant persists after user confirm',
        ],
        documentGrounded: true,
    });

    const compileResult = createCompileResultNode();
    const apologise = createApologiseNode();

    return new StateGraph(AdviceGenerationState)
        .addNode('tool_actor', toolActor)
        .addNode('compile_result', compileResult)
        .addNode('apologise', apologise)
        .addEdge('__start__', 'tool_actor')
        .addConditionalEdges('tool_actor', routeAfterToolActor, {
            tool_actor: 'tool_actor',
            compile_result: 'compile_result',
            apologise: 'apologise',
        })
        .addEdge('compile_result', '__end__')
        .addEdge('apologise', '__end__')
        .compile({ checkpointer });
}

/** Compiled advice-generation graph. */
export type AdviceGenerationAgent = ReturnType<typeof buildAdviceGenerationAgent>;
